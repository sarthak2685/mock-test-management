...
const SubscriptionForm = () => {
  ...
  useEffect(() => {
    setErrors({
      ...
      invalidEmail: formData.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email),
    });
  }, [formData]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (errors.passwordMismatch || errors.invalidPhone || errors.invalidEmail) return;
    setLoading(true);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="relative">
              ...
              {errors.passwordMismatch && (
                <p className="text-sm text-red-500">Passwords do not match. Please check and try again.</p>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-center items-center">
            <button
              ...
              className={`w-auto px-10 py-3 ${
                loading ? "bg-gray-400 cursor-not-allowed" : "bg-gradient-to-r from-blue-500 to-indigo-600"
